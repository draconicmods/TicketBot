...
@Command(label = "serial", usage = "serial <Serial Number>", description = "Sets the serial number field in an open ticket", minArgs = 1)
public class SerialCommand {
    ...
    public CommandResult onExecute(Member member, TextChannel channel, Message message, String label, List<String> args, SupportBot main) {
        ...
        if (message.getAuthor().getIdLong() == authorId) {
            // Build replacement embed message for the channel containing the new serial number.
            EmbedBuilder embedBuilder = new EmbedBuilder()
                    ...
                    .addField(originalMessage.getEmbeds().get(0).getFields().get(2).getName(), String.join(" ", args), true); // Serial
